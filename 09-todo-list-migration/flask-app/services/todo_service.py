...
class TodoService:
    ...
    def get_all_todos(self, user_id: int) -> List[dict]:
        """
        獲取用戶的所有 TODO（帶快取）
        ...
        def fetch_todos():
            todos = self.todo_repo.get_all_by_user(user_id)
            return [todo.to_dict() for todo in todos]
        
        # 使用快取（自動處理雪崩、穿透、擊穿）
        # 注意：對於列表查詢，不使用布隆過濾器，因為空列表是有效結果
        todos = self.cache.get_or_set(
            key=cache_key,
    ...
    def get_todo(self, todo_id: int, user_id: int) -> dict:
        """
        獲取單個 TODO（帶快取）
    ...
    def create_todo(self, user_id: int, title: str, description: Optional[str] = None) -> dict:
        ...
        Returns:
            dict: 創建的 TODO 資料
        """
        todo = self.todo_repo.create(user_id, title, description)
        
        # 使相關快取失效（創建後需要清除列表快取）
        self.cache.invalidate_user_todos(user_id)
        # 雖然是新創建的，但為了保險起見也清除單個 todo 快取
        self.cache.invalidate_todo(todo.id)
    ...
    def update_todo(
        ...
        todo = self.todo_repo.update(todo_id, user_id, **update_data)
        
        # 使相關快取失效
        self.cache.invalidate_todo(todo_id)
        self.cache.invalidate_user_todos(user_id)
    ...
    def delete_todo(self, todo_id: int, user_id: int) -> bool:
        ...
        Raises:
            NotFoundException: 如果 TODO 不存在
        """
        result = self.todo_repo.delete(todo_id, user_id)
